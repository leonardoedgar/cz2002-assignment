...
public class Hotel {
	 ...
	 * 
	 * @param roomConfig {Hashtable<String, Hashtable<String, String>>} the room
	 *                   configuration to built from.
	 */
	private void setupRooms(Hashtable<String, Hashtable<String, String>> roomConfig) {
		...
		for (String roomType : roomConfig.keySet()) {
			...
			for (int i = 1; i <= Integer.parseInt(roomDetail.get("numberOfRooms")); i++) {
				...
				roomDataPerLevel.put(finalRoomNo,
						new Room(finalRoomNo, Double.parseDouble(roomDetail.get("cost")), roomDetail.get("bedType"),
								roomDetail.get("wifi"), roomDetail.get("view"), roomDetail.get("smoking")));
			}
			;
	 ...
	 * @throws {GuestNotFoundException} when guest's name is not registered in the
	 *                                  hotel
	 */
	 ...
	 * @throws {GuestNotFoundException}            when guest's name is not
	 *                                             registered in the system
	 * @throws {GuestDetailUpdateFailureException} when system failed to update
	 *                                             guest's data due to data does not
	 *                                             exist
	 */
	...
	public void updateRoomDetails(String roomNo, String details, String newData)
		...
		case "status": {
			...
			if (!newData.equals(room.getStatus())) {
				...
				this.getReservationSystem().shiftReservation(this.getNumberOfRoomsByRoomType(roomType), roomType,
						delta);
	...
	public int checkDateClash(Date newStartDate, Date newEndDate, Date existingStartDate, Date existingEndDate) {
		...
		if (newStartDate.compareTo(existingStartDate) > 0) {
			...
			// if newStartDate after or on existingEndDate (Prev guest checked out), no
			// clash
		...
		if (newStartDate.compareTo(existingStartDate) < 0) {
			...
			// newEndDate before or on existingStartDate (new guest leaves before existing
			// guest comes), no clash
	...
	/**
	 * A function to check for number of guests staying in a roomType for a given
	 * date range
	 * 
	...
	private int checkHotelClash(java.util.Date startDate, java.util.Date endDate, String roomType) {
		...
		for (String key : tempTable.keySet()) {
			...
			if (tempTable.get(key).getGuest() != null) {
				...
				roomsClash = roomsClash + checkDateClash(startDate, endDate, tempGuest.getStartDateOfStay(),
						tempGuest.getEndDateOfStay()); // if dates clash, minus 1 from available rooms of that roomType
			}
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (!(reservationIdInDateRange.contains(tempRes.getReservationId()))) {
				...
				if (!tempRes.getStatus().contentEquals("checked-in")) {
					roomsClash += checkDateClash(startDate, endDate, tempRes.getDateOfCheckIn(),
							tempRes.getDateOfCheckOut());
				}
			}
	...
	/**
	 * A function to check if there are any rooms of a particular room type
	 * available
	 * 
	...
	public boolean checkRoomAvailability(java.util.Date startDate, java.util.Date endDate, String roomType)
		...
		// checkHotel method
		roomsLeftForDate = roomsLeftForDate - checkHotelClash(startDate, endDate, roomType)
				- checkReservationClash(startDate, endDate, roomType);
	...
	/**
	 * A function to check-in a guest who arrived at the hotel into a room chosen by
	 * the staff
	 * 
	...
	/**
	 * A function to check-in a guest with a reservation who arrived at the hotel
	 * into a room chosen by the staff
	 * 
	 * @param roomNo      {String} roomNo of an empty room
	 * @param guest       {Guest} object of the guest who wants to check-in
	 * @param roomType    {String} roomType chosen by the guest
	 ...
	 * @param orderMap {Hashtable<String, String>} the ordered items and quantity
	 *                 from the menu
	 ...
	 */
	public boolean checkOut(String roomNo, String guestName)
			throws RoomNotFoundException, GuestNotFoundException, RoomTypeNotFoundException {
		...
		for (String roomType : this.roomTable.keySet()) {
			for (String roomNo : this.roomTable.get(roomType).keySet()) {
				...
				if (roomStatusToRoomNoListTable.containsKey(roomStatus)) {
					...
					roomStatusToRoomNoListTable.put(roomStatus, new ArrayList<String>() {
						{
							add(roomNo);
						}
					});
			...
			if (sortedRoomNoList.size() == 0) {
				...
				for (int index = 0; index < sortedRoomNoList.size(); index++) {
					...
					if (Integer.parseInt(levelNoOfRoomInSortedList) > Integer.parseInt(levelNo)
							|| (Integer.parseInt(levelNoOfRoomInSortedList) == Integer.parseInt(levelNo) && Integer
									.parseInt(explicitRoomNoInSortedList) > Integer.parseInt(explicitRoomNo))) {
